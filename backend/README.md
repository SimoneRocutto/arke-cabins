...
Remember to edit also the `env.sh.eex` file which export `RELEASE_NODE=backend@${POD_IP}`, where `POD_IP` is inherited from the k8s pod. 
## Mailer
...
If you want to create our own strategy edit the `Backend.Provider.MyProvider` as you like
